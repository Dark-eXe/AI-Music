...
def generateMusic(inst, artist, user_tempo, measures, music_file, showSheetMusic=False):
    ...
    unique_x_path = find_file(unique_x_file, "../")
    xval_path = find_file(xval_file, "../")
    ...
    # programmer-defined timesteps (shouldn't mess with)
    no_of_timesteps = 32
    
    # Music Generation
    model = load_model(model_path)
    x_val = np.genfromtxt(xval_path)
    with open(unique_x_path, "r") as file:
...
def mid_button(inst, artist, tempo, numMeasures, inputFile, showSheetMusic):
    ...
    if not isinstance(numMeasures, int):
    ...
    generateMusic(inst, artist, tempo, numMeasures, inputFile, showSheetMusic) # Interact with the AI and generate the music.
